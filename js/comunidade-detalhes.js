...
class ComunidadeDetalhes {
    ...
    renderizar() {
        ...
        if (botoes) {
            ...
            if (isCriador) {
                botoes.innerHTML = `<button onclick="editarComunidade()" class="btn">✏️ Editar</button> <button onclick="deletarComunidade()" class="btn">🗑️ Deletar</button>`;
            } else if (isMembro) {
    ...
    renderPosts(isMembro) {
        ...
        let html = '';
        if (isMembro) {
            html += `<div class="form-novo-post"> <textarea id="novo-post-texto" placeholder="Escreva algo..."></textarea> <button onclick="adicionarPost()" class="btn">📤 Postar</button> </div>`;
        }
    ...
    renderPost(post, index) {
        ...
        return `<div class="post-item" id="post-${index}"> <div class="post-header"> <div class="post-autor"> <div class="avatar-post">${post.autor.charAt(0).toUpperCase()}</div> <div class="post-info"> <strong>${post.autor}</strong> <span class="post-data">${dataPost} ${horaPost}</span> </div> </div> </div> <div class="post-conteudo">${post.conteudo}</div> <div class="post-actions"> <button onclick="curtirPost(${index})" class="action-btn ${temCurtida ? 'curtido' : ''}"> ${temCurtida ? '❤️' : '🤍'} ${numCurtidas} </button> <button onclick="mostrarComentarios(${index})" class="action-btn"> 💬 ${comentarios.length} </button> </div> <div id="comentarios-${index}" class="comentarios" style="display: none;"> <div class="lista-comentarios"> ${comentarios.map((com, i) => ` <div class="comentario"> <strong>${com.autor}</strong>: ${com.texto} <small>${new Date(com.data).toLocaleDateString('pt-BR')}</small> </div> `).join('')} </div> <div class="form-comentario"> <input type="text" id="com-texto-${index}" placeholder="Comentar..."> <button onclick="adicionarComentario(${index})" class="btn-pequeno">Enviar</button> </div> </div> </div>`;
    }
