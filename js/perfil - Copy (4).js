// ╔════════════════════════════════════════════════════════════════════════════╗
// ║ PERFIL.JS - ORIGINAL COM ABA JOGOS CORRIGIDA ║
// ╚════════════════════════════════════════════════════════════════════════════╝
...
function setupAbas() {
    ...
    btns.forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            // ✨ NOVO: Carregar jogos quando aba for clicada
            if (id === 'jogos') {
...
function carregarComunidades(usuario) {
    ...
    minhasComunidades.forEach(com => {
        ...
        <div style="border-left: 3px solid #7e30ff; padding: 15px; background: rgba(45,0,80,0.3); border-radius: 8px;">
            ...
            <p style="color: #b895d4; font-size: 0.9rem; margin-bottom: 8px;">${com.descricao.substring(0, 80)}...</p>
...
// ✨ ✨ NOVA FUNÇÃO: CARREGAR JOGOS (PADRÃO BIBLIOTECA.JS) ✨ ✨
window.carregarJogos = function() {
    ...
    // Tentativa 1: localStorage específico do usuário
    ...
    // Tentativa 3: localStorage genérico
    if (!biblioteca) {
...
function abrirModalPerfil(usuario) {
    ...
    if (salvar) {
        salvar.addEventListener('click', function() {
            ...
            if (avatar && avatar !== usuario.avatar && avatar.startsWith('data:')) {
